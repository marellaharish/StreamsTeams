...
class MessageHandler {
    ...
    addSMSGroupDID(group_code, did_data, is_chat_message) {
        ...
        try {
            ...
            let data = this.sms_group_dids.get(String(group_code));

            if (data == null || data == undefined) {

                data = this.sms_group_dids.get(group_code);
            ...
            if (is_chat_message) {
                ...
                const index = data.findIndex((item) => {
                    ...
                    const existing_msg_to = JSON.parse(item.latest_message).to;
                    const current_msg_to = JSON.parse(did_data[0].latest_message).to;
    ...
    addSMSAssignedGroupsDIDList(group_code, response_data) {
        ...
        try {
            ...
            this.sms_assigned_groups_didlist.set((group_code * 1), response_data);
    ...
    getSMSAssignedGroupsDIDList(group_code) {
        ...
        try {
            ...
            if (this.sms_assigned_groups_didlist.get((group_code * 1))) {

                didsList = this.sms_assigned_groups_didlist.get((group_code * 1));
            }
        ...
        try {
            ...
            for (let i = 1; i <= this.sms_data.size; i++) {
                ...
                for (let j = 0; j <= message_array.length; j++) {
                    ...
                    if (message_array[j].phnumber === phnumber) {
    ...
    addContacts(contact_type, response_data) {
        ...
        try {
            ...
            this.contacts_list.set(contact_type, response_data);
    ...
    getContacts(contact_type) {
        ...
        return this.contacts_list.get(contact_type);
    }
